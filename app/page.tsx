...
const EXAMPLES_IMAGES = [
  { style: "Scarlett Johansson", before: "/examples/scarlett_johansson_avant.png", after: "/examples/scarlett_johansson_apres.png", mobile: true, mobileFirst: false },
  { style: "Mia Khalifa",   before: "/examples/mia_avant.png" , after: "/examples/mia_apres.png", mobile: true, mobileFirst: false },
  { style: "Leonardo DiCaprio",          before: "/examples/leonardo_dicaprio_avant.png", after: "/examples/leonardo_dicaprio_apres.png", mobile: true, mobileFirst: false },
    ...
    <div className="relative overflow-hidden py-0.5 sm:py-4">
      ...
      >
        {[...REVIEWS, ...REVIEWS].map((review, i) => (
          <div
            key={i}
            className="w-44 sm:w-80 flex-shrink-0 card border border-surface-border p-2 sm:p-5 rounded-xl sm:rounded-2xl"
          >
            <div className="flex items-center justify-between mb-1 sm:mb-3">
              <div className="flex gap-0.5 sm:gap-1">
                {Array.from({ length: review.stars }).map((_, s) => (
                  <Star key={s} className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 fill-yellow-400 text-yellow-400" />
                ))}
                {Array.from({ length: 5 - review.stars }).map((_, s) => (
                  <Star key={s} className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-white/20" />
                ))}
              </div>
              <Quote className="w-3 h-3 sm:w-4 sm:h-4 text-accent-violet/40" />
            </div>
            <p className="text-white/70 text-[10px] sm:text-sm leading-snug sm:leading-relaxed mb-1.5 sm:mb-4 line-clamp-2 sm:line-clamp-3">
              &ldquo;{review.text}&rdquo;
            </p>
            <div className="flex items-center gap-2 sm:gap-2.5">
              <div className="w-5 h-5 sm:w-8 sm:h-8 rounded-full bg-gradient-violet-neon flex items-center justify-center text-white text-[10px] sm:text-xs font-bold flex-shrink-0">
                {review.name.charAt(0)}
              </div>
              <div>
                <p className="text-white font-medium text-[11px] sm:text-sm leading-none">{review.name}</p>
                <p className="text-white/40 text-[9px] sm:text-xs mt-0.5">{review.city}</p>
              </div>
            </div>
          </div>
        ))}
...
function ExamplesGallery() {
  ...
  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 max-w-[20rem] sm:max-w-none mx-auto">
        {EXAMPLES_IMAGES.map((ex, i) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 bg-gradient-to-t from-black/70 to-transparent pointer-events-none">
              <p className="text-white text-[11px] sm:text-sm font-medium">{ex.style}</p>
            </div>
          </motion.div>
        ))}
      </div>
...
function SnapRougeTutoSection() {
  return (
    <section className="py-3 sm:py-6 px-4 sm:px-6 relative overflow-hidden">
      {/* Ambiance rouge */}
      <motion.div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[220px] sm:w-[360px] h-[120px] sm:h-[180px] rounded-full bg-red-500/8 blur-[100px] pointer-events-none"
        animate={{ scale: [1, 1.3, 1], opacity: [0.5, 1, 0.5] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
      />
      <div className="absolute -bottom-16 -right-16 w-56 h-56 rounded-full bg-accent-violet/8 blur-3xl pointer-events-none" />
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        >
          <span className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/30 text-red-400 text-[10px] sm:text-xs font-semibold px-2 sm:px-3 py-0.5 sm:py-1.5 rounded-full mb-1.5 sm:mb-2">
            <Flame className="w-3 h-3" />
            Tuto Snapchat
          </span>
          <h2 className="text-sm sm:text-xl font-bold mb-1 sm:mb-1.5">
            Envoyer un <span className="text-red-500">Snap Rouge</span> 🔥
          </h2>
          <p className="text-white/50 text-[10px] sm:text-sm max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3 mx-auto mb-2 sm:mb-4">
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="rounded-2xl border border-surface-border bg-surface p-2 sm:p-3"
          >
            <div className="flex items-center gap-2 mb-1 sm:mb-1.5">
              <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-md bg-purple-500/15 border border-purple-500/30 flex items-center justify-center text-[10px] sm:text-xs">💜</span>
              <p className="font-black text-xs sm:text-sm text-white/70">Snap violet</p>
            </div>
            <p className="text-white/45 text-[10px] sm:text-[11px] leading-snug">
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="rounded-2xl border border-red-500/40 bg-red-500/5 p-2 sm:p-3 relative overflow-hidden"
          >
            <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-red-500/15 blur-2xl pointer-events-none" />
            <div className="flex items-center gap-2 mb-1 sm:mb-1.5 relative">
              <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-md bg-red-500/15 border border-red-500/30 flex items-center justify-center text-[10px] sm:text-xs">🔴</span>
              <p className="font-black text-xs sm:text-sm text-red-400">Snap Rouge</p>
            </div>
            <p className="text-white/55 text-[10px] sm:text-[11px] leading-snug relative">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="hidden sm:flex flex-wrap justify-center gap-1.5 mx-auto mb-4"
        >
          {[
            ...
            <span key={label} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-red-500/10 border border-red-500/25 text-red-400 text-[10px] sm:text-[11px] font-medium">
        ...
        >
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-bold text-xs sm:text-sm transition-all shadow-lg shadow-red-500/25 hover:scale-[1.03] active:scale-[0.97]"
          >
            <Flame className="w-4 h-4" />
            Débloquer la technique complète
            <ArrowRight className="w-4 h-4" />
          </Link>
          <p className="text-white/30 text-[10px] sm:text-[11px] mt-1.5 sm:mt-2">
        ...
        return (
          ...
          >
            <button
              ...
              className="w-full flex items-center justify-between px-3 sm:px-5 py-2 sm:py-3.5 text-left group"
            >
              <span className={`font-semibold text-[11px] sm:text-sm transition-colors ${isOpen ? "text-white" : "text-white/80 group-hover:text-white"}`}>
                {item.q}
              </span>
              <ChevronDown
                className={`w-3.5 h-3.5 sm:w-5 sm:h-5 flex-shrink-0 ml-3 sm:ml-4 transition-all duration-300 ${
            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <p className="px-3 sm:px-5 pb-2.5 sm:pb-4 text-white/60 leading-relaxed text-[10px] sm:text-sm">
                    {item.a}
                  </p>
                </motion.div>
              )}
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <section className="relative sm:min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-3 sm:py-32 text-center">
          <motion.div
          ...
          <motion.div
            ...
            className="grid grid-cols-2 gap-3 sm:gap-6 max-w-[14rem] sm:max-w-sm mx-auto"
          >
            {STATS.map((stat) => (
      ...
      {/* ══ GALERIE EXEMPLES ══════════════════════════════════════════════ */}
      <section className="pt-1 pb-4 sm:py-24 px-4 sm:px-6 relative overflow-hidden">
        {/* Fond teinté */}
      ...
      {/* ══ ESSAI GRATUIT (PUBS LINKVERTISE) ══════════════════════════════ */}
      <div>
        <FreeCreditsAds variant="section" />
      </div>
      ...
      {/* ══ SÉPARATEUR ANIMÉ DÉMO → AVIS ════════════════════════════════ */}
      <div className="relative h-6 sm:h-28 overflow-hidden pointer-events-none select-none">
        {/* Ligne lumineuse */}
        <motion.div
      ...
      {/* ══ AVIS CLIENTS ══════════════════════════════════════════════════ */}
      <section className="py-3 sm:py-20 overflow-hidden">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          ...
          <h2 className="text-base sm:text-5xl font-bold mb-1 sm:mb-3">
      ...
      {/* ══ FAQ ═══════════════════════════════════════════════════════════ */}
      <section className="py-3 sm:py-12 px-4 sm:px-6 bg-surface/20">
        <div className="max-w-2xl mx-auto">
          ...
          >
            <h2 className="text-sm sm:text-3xl font-bold mb-1 sm:mb-2">
              Questions <span className="gradient-text">fréquentes</span>
            </h2>
            <p className="text-white/50 text-[10px] sm:text-base">
          ...
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-center text-white/40 text-[10px] sm:text-sm mt-2 sm:mt-8"
          >
            Une autre question ?{" "}
